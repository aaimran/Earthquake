...
def update_graphs(time_value, sim_data, tau_val):
    ...
    # Global bounds for velocity plots.
    v_all = np.concatenate((DomainOutput_l[:, :, 0].flatten(), DomainOutput_r[:, :, 0].flatten()))
    v_min, v_max = np.min(v_all), np.max(v_all)
    
    # Global bounds for stress plots, adjusted by initial stress.
    s_all_adjusted = np.concatenate((DomainOutput_l[:, :, 1].flatten(), DomainOutput_r[:, :, 1].flatten())) + initial_stress
    s_min, s_max = np.min(s_all_adjusted), np.max(s_all_adjusted)
    ...
    # Here we add the initial stress value to the simulation stress data so that the stress
    # distribution starts from the initial stress.
    ...
    fig_timeseries.update_yaxes(range=[np.min(slip_vector), np.max(slip_vector)], row=1, col=1)
    fig_timeseries.update_yaxes(range=[np.min(sliprate_vector), np.max(sliprate_vector)], row=2, col=1)
    fig_timeseries.update_yaxes(range=[np.min(traction_vector), np.max(traction_vector)], row=3, col=1)
